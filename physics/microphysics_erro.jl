...
function compute_dqpdt_sam_micro!(
    uaux, qe, Tabs, qn, qc, qi, qr, qs, qg, qsatt, S_micro, npoin, MicroConst, PhysConst, NSD, lpert
)
    # --- WRF/NUMA Kessler logic, preserving argument names/signature ---

    # Constants from WRF/NUMA Kessler
    c1 = 0.001
    c2 = 0.001
    c3 = 2.2
    c4 = 0.875
    dt = PhysConst.dt  # or however you supply your timestep

    cp = PhysConst.cp
    xlv = MicroConst.xlv
    ep2 = MicroConst.ep2
    svp1 = MicroConst.svp1
    svp2 = MicroConst.svp2
    svp3 = MicroConst.svp3
    svpt0 = MicroConst.svpt0
    ρwater = MicroConst.ρwater
    p00 = PhysConst.p00
    rgas = PhysConst.rgas
    ...
    # SEDIMENTATION PREP
    dz = 1000.0  # <-- TODO: supply correct dz (vertical grid spacing) for your grid
    max_cr_sedimentation = 0.75

    # Use existing storage (optional, for batch ops)
    np = npoin

    # Temporary arrays for sedimentation
    prod   = zeros(Float64, np)
    prodk  = zeros(Float64, np)
    vt     = zeros(Float64, np)
    vtden  = zeros(Float64, np)
    rdzk   = zeros(Float64, np)
    crmax = 0.0

    # Main loop
    for ip in 1:np
        # Get density
        ρ = lpert ? (uaux[ip,1] + qe[ip,1]) : uaux[ip,1]
        rdzk[ip]  = 1.0 / dz
        prodk[ip] = qr[ip]
        qrr       = max(0.0, qr[ip] * 0.001 * ρ)
        vtden[ip] = sqrt(1.123 / ρ)
    ...
    while nfall > 0
        ...
        # Euler step for sedimentation
        for ip in 1:np
            prodk[ip] -= dtfall * prodk[ip] * vt[ip] * rdzk[ip]
        end
        nfall -= 1
    end
    for ip in 1:np
    ...
    # --- Main Microphysics: WRF/NUMA core logic
    for ip in 1:np
        # Get state
        ρ = lpert ? (uaux[ip,1] + qe[ip,1]) : uaux[ip,1]
        Tabs_ip = Tabs[ip]       # potential temperature or temperature
        p = lpert ? (uaux[ip,end] + qe[ip,end]) : uaux[ip,end]
        if isnan(p) || p <= 0
            p = 90000.0
        end

        # 1. Autoconversion/accretion: cloud to rain
        factorn = 1.0 / (1.0 + c3 * dt * max(0.0, qr[ip])^c4)
        qrprod = qc[ip] * (1.0 - factorn) + factorn * c1 * dt * max(qc[ip] - c2, 0.0)
        rcgs = 0.001 * ρ

        qc[ip] = max(qc[ip] - qrprod, 0.0)
        qr[ip] = qr[ip] + prod[ip] - qr[ip]  # prod is updated from sedimentation
        qr[ip] = max(qr[ip] + qrprod, 0.0)

        # THIS IS THE S_micro (autoconversion) for source terms
        S_micro[ip] = c1 * max(qc[ip] - c2, 0.0)

        # 2. Thermodynamic pressure and temperature
        pii = (p / p00)^(rgas / cp)
        temp = Tabs_ip * pii
        pressure = p
        gam = 2.5e6 / (cp * pii)

        # 3. Compute saturation and condensation/evaporation
        es = 1000.0 * svp1 * exp(svp2 * (temp - svpt0) / (temp - svp3))
        qvs = ep2 * es / (pressure - es)
        qv = lpert ? (qe[ip,3] / ρ) : (qe[ip,3] / ρ)  # Adjust for your system

        prod_phase = (qv - qvs) / (1.0 + pressure / (pressure - es) * qvs * f5 / (temp - svp3)^2)

        # 4. Rain evaporation (WRF formula)
        ern = min(
            dt * (
                ((1.6 + 124.9 * (rcgs * qr[ip])^0.2046) * (rcgs * qr[ip])^0.525) /
                (2.55e8 / (pressure * qvs) + 5.4e5)
            ) * (qvs > qv ? 1.0 : 0.0) / (rcgs * qvs),
            max(-prod_phase - qc[ip], 0.0),
            qr[ip]
        )

        # 5. Update microphysics (mass budget)
        product = max(prod_phase, -qc[ip])
        Tabs[ip] += gam * (product - ern)
        # update vapor (using your array conventions, adjust as needed)
        qe[ip,3] = max(qe[ip,3] - product + ern, 0.0) * ρ
        qc[ip] += product
        qr[ip] -= ern

        # The rest (ice/snow/graupel) can be set to zero if single-moment Kessler only
        qi[ip] = 0.0
        qs[ip] = 0.0
        qg[ip] = 0.0
    end
...
function kessler(mp::St_Microphysics, params, q, qref, mesh)
    ...
    temp3 = 0.0
    temp4 = 0.0
    println("=== Entering Kessler microphysics step ===")
    kessler_nocolumn!(mp,
                  params,
                  mesh.npoin,
                  @view(q[:,1]),               # ρ
...
#=function add_micro_precip_sources!(mp::St_Microphysics, flux_lw, flux_sw, T, S_micro, S, q, qn, qe, ::NSD_2D, ::PERT)
    return nothing
end=#
...
function add_micro_precip_sources!(mp::St_Microphysics, flux_lw, flux_sw, T, S_micro, S, q, qn, qe, ::NSD_2D, ::PERT)
    ...
    # If you are using perturbation variables (q, qn, qe), adapt as necessary.
    # This is a safe pattern if your code is structured with q[1]=rho, q[5]=qt, q[6]=qp, etc.
    ρ        = q[1] + qe[1]
    qt       = (q[5] + qe[5]) / ρ
    qp       = (q[6] + qe[6]) / ρ
    qv       = qt - qn
    ...
    # --- BUOYANCY SOURCE (for momentum eqn, if relevant) ---
    S[3] += PhysConst.g * (0.61 * ρqv_pert - ρ * (qn + qp))

    # --- RADIATIVE SOURCE (for θ or enthalpy eqn) ---
    S[4] += ρ * (flux_lw - flux_sw)

    # --- MICROPHYSICS SOURCE/LOSS (cloud/rain) ---
    S[5] += -ρ * S_micro    # Source for cloud: REMOVE due to autoconversion
    S[6] += ρ * S_micro     # Source for rain: ADD due to autoconversion
...
function kessler_nocolumn!(mp::St_Microphysics,
    ...
    println("DEBUG: kessler_nocolumn! called")

    # Saturation and latent heat constants (from your param structs)
    physConst = PhysicalConst{Float64}()
    cp        = physConst.cp
    ...
    ngl     = params.mesh.ngl
    nop     = params.mesh.nop
    nelz    = params.mesh.nelz
    zbottom = params.zmin
    ztop    = params.zmax

    # Aliases to struct arrays
    prod   = mp.prod
    prodk  = mp.prodk
    vt     = mp.vt
    vtden  = mp.vtden
    ...
    rainnc  = mp.rainnc
    rainncv = mp.rainncv
    S_micro = mp.S_micro  # <--- ADD THIS

    # Pre-calc
    f5 = svp2 * (svpt0 - svp3) * xlv / cp

    dz = (ztop - zbottom) / max(nelz * nop, 1)
    crmax = 0.0
    ...
    # For NUMA: q0 is working copy for time-split
    q0 = similar(prodk)
    for ip = 1:npoin
        q0[ip] = prodk[ip]
    end

    # SEDIMENTATION (time-split for stability)
    while nfall > 0
        time_sediment -= dtfall

        # Rain accumulation at ground (optional, can be adjusted per your domain)
        for ip = 1:npoin
        ...
        # Update prodk using a simplified RK or Euler; see NUMA's ti_rk35_production_no_column
        # For a first pass, use Euler:
        for ip = 1:npoin
            prodk[ip] -= dtfall * prodk[ip] * vt[ip] * rdzk[ip]
        end

        # Recompute sedimentation stability if needed
        if nfall > 1
            crmax = 0.0
            for ip = 1:npoin
                qrr = max(0.0, prodk[ip] * 0.001 * ρ[ip])
                vt[ip] = 36.34 * (qrr^0.1364) * vtden[ip]
                crmax = max(vt[ip] * time_sediment * rdzk[ip], crmax)
            end
        ...
        # Update q0 for next split (optional, if needed by your RK structure)
        for ip = 1:npoin
            q0[ip] = prodk[ip]
        end
    end

    # PHASE-CHANGE + TEMPERATURE UPDATE + S_micro (autoconversion rate)
    for ip = 1:npoin
        # 1. Autoconversion/accretion: cloud to rain
        factorn = 1.0 / (1.0 + c3 * dt * max(0.0, qr[ip])^c4)
        qrprod = qc[ip] * (1.0 - factorn) + factorn * c1 * dt * max(qc[ip] - c2, 0.0)
        ...
        S_micro[ip] = c1 * max(qc[ip] - c2, 0.0)  # Kessler autoconversion (cloud → rain)

        # 2. Thermodynamic pressure and temperature
        p = qref[4, ip]  # Assume reference pressure is in qref[4, :]
        pii = (p / p00)^(rgas / cp)
        temp = t[ip] * pii
        pressure = p
        gam = 2.5e6 / (cp * pii)
        ...
        es = 1000.0 * svp1 * exp(svp2 * (temp - svpt0) / (temp - svp3))
        qvs = ep2 * es / (pressure - es)
        prod_phase = (qv[ip] - qvs) / (1.0 + pressure / (pressure - es) * qvs * f5 / (temp - svp3)^2)
    ...
    println("qc: min=", minimum(qc), " max=", maximum(qc), " mean=", mean(qc))
    println("qr: min=", minimum(qr), " max=", maximum(qr), " mean=", mean(qr))
    println("prod: min=", minimum(prod), " max=", maximum(prod), " mean=", mean(prod))
    println("rainnc: min=", minimum(rainnc), " max=", maximum(rainnc), " mean=", mean(rainnc))
    println("rainncv: min=", minimum(rainncv), " max=", maximum(rainncv), " mean=", mean(rainncv))
end
