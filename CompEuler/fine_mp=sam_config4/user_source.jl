...
function user_source!(S, q, qe, npoin, ::CL,::TOTAL; neqs=1,x=0.0, y=0.0, ymin=0.0, ymax=24000.0, ngl=5, nely=10,xmin = -75000, xmax =75000)  # CHANGED: ymax and xmin/xmax
    ...
    dbl = ymax - y
    zs = 16000.0#ymax - 16000.0
    dsx = (xmax - xmin)/(nely*(ngl - 1))# equivalent grid spacing
    dbx = min(xmax - x,x-xmin) 
    xr = 75000.0  # CHANGED: Match xmax
    xl = -75000.0  # CHANGED: Match xmin
...
function user_source!(S, q, qe, npoin, ::CL,::PERT; neqs=1,x=0.0, y=0.0, ymin=0.0, ymax=24000.0, ngl=5, nely=10,xmin = -75000, xmax =75000, ctop_mult=0.02)  # CHANGED: ymax and xmin/xmax
    ...
    dsx = (xmax - xmin)/(nely*(ngl - 1))# equivalent grid spacing
    dbx = min(xmax - x,x-xmin)
    xr = 40000.0  # Keep this - it's INSIDE your domain for interior damping
    xl = -40000.0  # Keep this - it's INSIDE your domain for interior damping
    
    if (y >= zs)#nsponge_points * dsy) #&& dbl >= 0.0)
        betay_coe =  sinpi(0.5*(y-zs)/(ymax-zs))^2  # CHANGED: Use ymax instead of 24000.0
    else
        betay_coe = 0.0
    end
    #if (abs(x) <=xmin)
    ...
    ctop = ctop_mult*min(ctop,1)  # CHANGED: Use parameter instead of hardcoded 0.25
    cxr  = min(cxr,1)
    cxl  = min(cxl,1)
    cs = 1.0 - (1.0 -ctop)*(1.0-cxr)*(1.0 - cxl)
